...
def chequeo(accion):
    ...
    grafica_list.to_csv(r'./' + str(accion) + '.csv', index=False, header=True)
    return grafica_list
...
def comparar():
    ...
    list_of_files = glob.glob('./comparaciones/*.csv')
    latest_file = max(list_of_files, key=os.path.getctime)
    latest_file = latest_file.replace("./comparaciones\\", "")
    ...
    bolsita_list.to_csv(r'./comparaciones/comparacion de' + time.strftime("%d.%m.%Y-%H.%M.%S") + '.csv', index=False, header=True)
    df = bolsita_list
    # df = grafica_list
...
def nuevo_archivo_base():
    ...
    tabla.title("Tabla")
    df = chequeo("base")
    df.to_csv(r'./respaldos/base de ' + time.strftime("%d.%m.%Y-%H.%M.%S") + '.csv', index=False, header=True)
    # df = grafica_list
